...
class FramelessWindow : public T {
   public:
    ...
    FramelessWindow(bool translucentBackground,
   ...
   protected:
    virtual void listenAllMessages(QWidget* widget) {
        if (!widget)
            return;
    ...
    virtual void setAllMouseTracking(QWidget* widget) {
        if (!widget)
            return;
